                         ...
                         const struct hmap *ls_ports)
{
    ...
    VECTOR_FOR_EACH_PTR (&lb_vip->backends, backend) {
        ...
        ovn_lflow_add(lflows, op->od, S_SWITCH_OUT_LB,
                      130, ds_cstr(&match), ds_cstr(&action), lflow_ref,
                      WITH_HINT(&lb->nlb->header_));
        ds_clear(&match);
        ds_clear(&action);
                            ...
                            const struct svc_monitors_map_data *svc_mons_data)
{
    ...
    const struct ovn_lb_backend *backend;
    size_t i = 0;
    /* тут было бы заебись переделать на какой0то цикл по локальным бекендам чтобы два раза не проверять пренадлежность к роутеру */
    VECTOR_FOR_EACH_PTR (&lb_vip->backends, backend) {
        struct ovn_northd_lb_backend *backend_nb =
                &lb_vip_nb->backends_nb[i++];
        struct ds match = DS_EMPTY_INITIALIZER;
...
static bool
l3dgw_port_has_associated_evpn_vni(const struct ovn_port *op)
{
    return lrp_is_l3dgw (op) && op->peer && op->peer->od->has_evpn_vni;
}
...
consider_l3dgw_port_is_centralized(struct ovn_port *op)
{
    ...
    if (l3dgw_port_has_associated_evpn_vni(op)) {
        return false;
    }
