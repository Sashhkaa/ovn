                ...
                const struct sbrec_port_binding *sb)
{
    ...
    op->l3dgw_port = op->cr_port = op->primary_port = NULL;
...
ovn_port_destroy(struct hmap *ports, struct ovn_port *port)
{
    if (port) {
        ...
        if (port->od && (!port->l3dgw_port && !port->primary_port)) {
                    ...
                    struct lflow_table *lflows)
{
    ...
    for (size_t i = 0; i < op->nbsp->n_mirror_rules; i++) {
        ...
        struct ovn_port *target_port = ovn_port_find(ls_ports,
                            ovn_mirror_port_name(ovn_datapath_name(op->od->sb),
                                                 mirror->sink));
        /* Mirror serving port was not created
         * because the target port does not exist.
         */
        if (!target_port) {
            continue;
        }
