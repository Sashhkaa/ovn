...
ovn_port_destroy(struct hmap *ports, struct ovn_port *port)
{
    if (port) {
        ...
        if (port->od && (!port->l3dgw_port && !port->mirror_source_port)) {
...
/*
 * Traffic directed to the target port will bypass connection tracking checks.
 */
static void
