 ...
 * port's address. Read back when building the ICMP redirect packet. Only set
 * in IP_ROUTING_PRE for ports with options:icmp_redirect=true. */
...
static void
build_lswitch_rport_arp_req_flow_centralized_routing(
...
static void
build_lswitch_rport_arp_req_flow_distributed_routing(
                                 ...
                                 struct lflow_ref *lflow_ref)
{
    ...
    bool is_centralized_routing =
        lsp_has_centralized_routing(switch_op);
                                             ...
                                             struct lflow_ref *lflow_ref)
{
    ...
    VECTOR_FOR_EACH (&od->router_ports, switch_op) {
        if (lsp_has_centralized_routing(switch_op)) {
            continue;
        }
    ...
    ovn_lflow_add(lflows, od, S_SWITCH_IN_L2_LKUP, 73,
...
/* Logical router ingress table IP_ROUTING_PRE, per port (priority 100):
 * selects the route table of the port and, if the port sends ICMP
 * redirects, saves the original eth.src. */
static void
build_ip_routing_pre_port_lflow(struct ovn_datapath *od,
                                struct lflow_table *lflows,
                                ...
                                struct lflow_ref *lflow_ref)
{
    ...
    bool icmp_redirect = smap_get_bool(&lrp->options, "icmp_redirect", false);
                            ...
                            const struct svc_monitors_map_data *svc_mons_data)
{
    ...
    bool ipv4 = lb_vip->address_family == AF_INET;
    const char *ip_match = ipv4 ? "ip4" : "ip6";
    const char *reg_lb_ip = ipv4 ? REG_LB_IPV4 : REG_LB_IPV6;
    ...
    VECTOR_FOR_EACH_PTR (&lb_vip->backends, backend) {
        ...
        if (ipv4 ? !lrp->lrp_networks.n_ipv4_addrs
                 : !lrp->lrp_networks.n_ipv6_addrs) {
            continue;
        }
        ...
        ds_put_format(&match, " && %s == %s", reg_lb_ip, backend->ip_str);
        ds_put_format(&action, "ip.ttl--; "
                      ...
                      ipv4 ? REG_NEXT_HOP_IPV4 : REG_NEXT_HOP_IPV6,
                      backend->ip_str,
                      ipv4 ? REG_SRC_IPV4 : REG_SRC_IPV6,
                      ipv4 ? lrp->lrp_networks.ipv4_addrs[0].addr_s
                           : lrp->lrp_networks.ipv6_addrs[0].addr_s,
                      lrp->lrp_networks.ea_s, lrp->json_key, ipv4);
        ...
        /* The reply from the backend has to be lookuped from conntrack in
         * the ingress pipeline of the backend's own switch, so that it is
         * unSNATed back to the VIP. */
        if (op->od->lb_with_stateless_mode) {
            ds_clear(&match);
            ds_clear(&action);
            ds_put_format(&match, "%s.src == %s", ip_match, backend->ip_str);
            if (backend->port) {
                ds_put_format(&match, " && %s.src == %"PRIu16, lb->proto,
                              backend->port);
            }
            ds_put_cstr(&action, "ct_lb_mark;");
            ovn_lflow_add(ctx->lflows, op->od, S_SWITCH_IN_PRE_STATEFUL, 102,
                          ds_cstr(&match), ds_cstr(&action), lflow_ref,
                          WITH_HINT(&lb->nlb->header_));
        }
    }
    ...
    if (n_members > 1) {
        ds_put_format(&action, "%s = select(values=(%s)", reg_lb_ip,
                      ds_cstr(&members));
        ...
        ds_put_format(&action, "%s = %s; next;", reg_lb_ip,
                      ds_cstr(&members));
    }
    ovn_lflow_add(ctx->lflows, od, S_ROUTER_IN_CT_EXTRACT, 130,
                  ds_cstr(&match), ds_cstr(&action), lflow_ref,
                                   ...
                                   struct lflow_ref *lflow_ref)
{
    ...
    if (lrp_has_centralized_routing(op)) {
        ...
        struct simap *route_tables)
{
    ...
    for (int i = 0; i < od->nbr->n_ports; i++) {
        build_ip_routing_pre_port_lflow(od, lflows, od->nbr->ports[i],
                                        route_tables, NULL);
    }
...
build_icmp_redirect_flows_for_lrouter_port(
        ...
        struct ds *match, struct ds *actions)
{
    if (!smap_get_bool(&op->nbrp->options, "icmp_redirect", false)) {
        return;
    }

    /* No Redirect in reply to a Redirect (RFC 1122 3.2.2). */
    ds_clear(match);
    ds_put_format(match, "inport == %s && icmp4.type == 5", op->json_key);
