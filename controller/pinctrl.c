...
#define ICMP_REDIRECT_GC_INTERVAL_MS (60 * 1000)
                       ...
                       long long int now)
{
    ...
    if (!e) {
        if (hmap_count(&icmp_redirect_map) >= ICMP_REDIRECT_MAP_MAX_SIZE) {
            /* Too many senders to keep track of.  Fall back to the COPP
             * meter alone. */
...
icmp_redirect_map_gc(long long int now)
{
    ...
    next_gc = now + ICMP_REDIRECT_GC_INTERVAL_MS;
...
static bool is_icmpv4_error(const struct icmp_header *ih)
{
    switch (ih->icmp_type) {
...
/* Called in the pinctrl_handler thread context. */
static void
                                       ...
                                       struct ofpbuf *continuation)
{
    ...
    if (!is_icmpv4_error(ih)) {
        static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(1, 5);
        VLOG_WARN_RL(
            &rl, "put_icmp4_inner_ip4_src action on non-ICMPv4 error packet");
        goto exit;
    }
